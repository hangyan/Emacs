...
;; writer-typewriter.elc in your .emacs or other tartup file.
...
(defvar writer-keyclick-noise "/home/bnewell/data/elisp/writer-mode/sounds/keyclick.wav")
(defvar writer-marginbell-noise "/home/bnewell/data/elisp/writer-mode/sounds/marginbell.wav")
(defvar writer-linefeed-noise "/home/bnewell/data/elisp/writer-mode/sounds/linefeed.wav")
...
;; Play the keyclick sound maybe, but not for spacebar.
	   ...
	   (if (> writer-input-charno 32)
	     (start-process-shell-command "writer-noise" nil (format "%s %s" writer-play-command writer-keyclick-noise))
           )

;; See if we are at the margin.  If so play the bell sound maybe.

	   (if (= (current-column) writer-margin-column)
	         (start-process-shell-command "writer-noise" nil (format "%s %s" writer-play-command writer-marginbell-noise))
           )
         )
...
;; Play the carriage return sound maybe.
   	       ...
   	       (start-process-shell-command "writer-noise" nil (format "%s %s" writer-play-command writer-linefeed-noise))
             )
...
(defun writer-make-click-maybe ()
  (if this-command
      (if (equal 'self-insert-command this-command)
          (progn
            (start-process-shell-command "writer-noise" nil (format "%s %s" writer-play-command writer-keyclick-noise))
            (if (= (current-column) writer-margin-column)
                (start-process-shell-command "writer-noise" nil (format "%s %s" writer-play-command writer-marginbell-noise))
        ...
        (if (equal 'newline-and-indent this-command)
            (start-process-shell-command "writer-noise" nil (format "%s %s" writer-play-command writer-linefeed-noise))
          )
    )
  )
