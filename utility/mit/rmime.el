...
(defvar rmime-save-dir nil
  "*Default directory for saving files.  nil means current directory.")
...
(defalias 'rmime-application/pgp-encrypted 'rmime-text/plain)
...
(defalias 'rmime-multipart/encrypted 'rmime-multipart/mixed)
...
(defun rmime-ftp (parameters anon)
  (let ((name      (cdr (assoc "name"      parameters)))
	...
	(control   (make-temp-name "/var/tmp/ftp"))
	(data      (make-temp-name "/var/tmp/mime")))
...
(defun rmime-octet-action (name content encoding)
  (let ((filename (read-file-name "Save transmitted file as: " rmime-save-dir nil nil name)))
    (save-excursion
      (set-buffer (or (get-file-buffer filename) (create-file-buffer filename)))
      (erase-buffer)
...
;;   (let ((filename (make-temp-name "/var/tmp/metamail")))
...
(defun rmime-insert-content (binaryp encoding content)
  (cond ((member encoding rmime-transparent-encodings)
	...
	((and rmime-mimencode-program (string= encoding "base64"))
	 (rmime-decode-content
	  binaryp rmime-mimencode-program (if binaryp '("-u") '("-u" "-p")) content))

	((and rmime-mimencode-program (string= encoding "quoted-printable"))
	 (rmime-decode-content
	  binaryp rmime-mimencode-program '("-u" "-q")                      content))

	((and rmime-uufilter-program  (string= encoding "x-uue"))
	 (rmime-decode-content
	  binaryp rmime-uufilter-program  '("-s")                           content))

	((and rmime-uufilter-program  (string= encoding "x-uuencode"))
	 (rmime-decode-content
	  binaryp rmime-uufilter-program  '("-s")                           content))
...
(defun rmime-decode-content (binaryp program args content)
  (let ((filename (make-temp-name "/var/tmp/mime")))
    (unwind-protect
	;; coding-system-for-read should be set depending on the charset
	;; argument to the Content-Type header
	(let ((coding-system-for-read (if binaryp 'no-conversion)))
	  (rmime-write-content filename content)
...
(defun rmime-next-object ()
  (interactive)
  (if rmime-mode
      (let ((pos (next-single-property-change (point) 'rmime-markup-face)))
...
(defadvice rmail-resend (around rmime activate)
  ...
  (let ((headers-were-pruned (rmail-msg-is-pruned))
	...
	(progn
	  ...
	  (if hook-was-there (remove-hook 'rmail-show-message-hook 'rmime-format))
...
(defadvice undigestify-rmail-message (around rmime activate)
  "Temporarily disable rmime-mode and show headers before undigestifying."
  (let ((rmime-mode-was-on rmime-mode)
	(hook-was-there (memq 'rmime-format rmail-show-message-hook)))
    (unwind-protect
	(progn
...
(defadvice unforward-rmail-message (around rmime activate)
  "Temporarily disable rmime-mode and show headers before unforwarding."
  (let ((rmime-mode-was-on rmime-mode)
	(hook-was-there (memq 'rmime-format rmail-show-message-hook)))
    (unwind-protect
	(progn
